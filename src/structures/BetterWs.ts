...
class BetterWs extends EventEmitter {
	...
	private _internal: { reconnectPromise: Promise<unknown> | null; closePromise: Promise<unknown> | null; zlib: import("zlib").Inflate | null; };
	...
	public sendMessage(data: import("../Types").IWSMessage) {
		const internal = this._internal;
		if(internal.reconnectPromise) return internal.reconnectPromise.then(() => this.sendMessage(data));
		...
		case 2: {
			...
			if(this.compress) {
				...
				if(message[l - 4] !== 0 || message[l - 3] !== 0 || message[l - 2] !== 255 || message[l - 1] !== 255) {
					this.emit("error", new Error("discord actually does send fragmented zlib messages. if you see this error let me know"));
				}
				// @ts-ignore
				z.close = z._c;
				// @ts-ignore
